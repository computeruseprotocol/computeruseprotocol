...
class TestPruneEdgeCases:
    ...
    def test_offscreen_interactive_named_kept(self):
        ...
        tree = [_make_node("e0", "button", "Send",
                           states=["offscreen"], actions=["click"])]
        pruned = prune_tree(tree)
        assert len(pruned) == 1
    ...
    def test_compact_attributes_heading_level(self):
        ...
        env = _make_envelope([_make_node(
            "e0", "heading", "Title", attributes={"level": 2},
        )])
        text = serialize_compact(env)
        assert "(L2)" in text
...
class TestOutputTruncation:
    def test_output_truncated_at_max_chars(self):
        ...
        nodes = [_make_node(f"e{i}", "button", f"Button {i}" * 10)
                 for i in range(200)]
        env = _make_envelope(nodes)
...
class TestChromeStripping:
    def test_scrollbar_subtree_skipped(self):
        ...
        tree = [_make_node("e0", "window", "App", children=[
            _make_node("e1", "scrollbar", "Scroll", actions=["increment", "decrement"], children=[
    ...
    def test_titlebar_subtree_skipped(self):
        ...
        tree = [_make_node("e0", "window", "App", children=[
            _make_node("e1", "titlebar", "App", children=[
    ...
    def test_status_bar_subtree_skipped(self):
        ...
        tree = [_make_node("e0", "window", "App", children=[
            _make_node("e1", "status", "Status Bar", children=[
    ...
    def test_zero_size_with_actions_skipped(self):
        ...
        tree = [_make_node("e0", "button", "Invisible", actions=["click"],
...
class TestViewportClipping:
    def test_offscreen_child_of_scrollable_clipped(self):
        ...
        tree = [_make_node("e0", "list", "Items",
                           bounds={"x": 0, "y": 0, "w": 400, "h": 200},
                           actions=["scroll"], children=[
            # Visible (within 0-200 y range)
            _make_node("e1", "listitem", "Item 1",
                       bounds={"x": 0, "y": 0, "w": 400, "h": 50}),
            _make_node("e2", "listitem", "Item 2",
                       bounds={"x": 0, "y": 50, "w": 400, "h": 50}),
            # Offscreen below (y >= 200)
            _make_node("e3", "listitem", "Item 5",
                       bounds={"x": 0, "y": 300, "w": 400, "h": 50}),
            _make_node("e4", "listitem", "Item 6",
                       bounds={"x": 0, "y": 350, "w": 400, "h": 50}),
        ])]
        pruned = prune_tree(tree)
        assert len(pruned) == 1
        container = pruned[0]
    ...
    def test_partially_visible_element_kept(self):
        ...
        tree = [_make_node("e0", "list", "Items",
            ...
            _make_node("e1", "listitem", "Partial",
                       bounds={"x": 0, "y": 180, "w": 400, "h": 50}),
        ])]
        pruned = prune_tree(tree)
        container = pruned[0]
        assert len(container["children"]) == 1
    ...
    def test_non_scrollable_does_not_clip(self):
        ...
        tree = [_make_node("e0", "group", "Panel", actions=["click"],
                           bounds={"x": 0, "y": 0, "w": 400, "h": 200},
                           children=[
            # This is outside the parent bounds but parent is NOT scrollable
            _make_node("e1", "button", "Far Away",
...
class TestClippedHints:
    def test_clipped_hint_shows_count(self):
        ...
        tree = [_make_node("e0", "list", "Items",
                           bounds={"x": 0, "y": 0, "w": 400, "h": 100},
                           actions=["scroll"], children=[
            _make_node("e1", "listitem", "Visible",
                       bounds={"x": 0, "y": 0, "w": 400, "h": 50}),
            _make_node("e2", "listitem", "Hidden 1",
                       bounds={"x": 0, "y": 200, "w": 400, "h": 50}),
            _make_node("e3", "listitem", "Hidden 2",
                       bounds={"x": 0, "y": 250, "w": 400, "h": 50}),
        ])]
    ...
    def test_clipped_hint_direction_both(self):
        ...
        tree = [_make_node("e0", "list", "Items",
            ...
            _make_node("e1", "listitem", "Above",
                       bounds={"x": 0, "y": 0, "w": 400, "h": 50}),
            _make_node("e2", "listitem", "Visible",
                       bounds={"x": 0, "y": 120, "w": 400, "h": 50}),
            _make_node("e3", "listitem", "Below",
                       bounds={"x": 0, "y": 300, "w": 400, "h": 50}),
        ])]
        env = _make_envelope(tree)
        text = serialize_compact(env)
    ...
    def test_hint_indentation(self):
        ...
        tree = [_make_node("e0", "window", "App", children=[
            _make_node("e1", "list", "Items",
                       bounds={"x": 0, "y": 0, "w": 400, "h": 100},
                       actions=["scroll"], children=[
                _make_node("e2", "listitem", "Visible",
                           bounds={"x": 0, "y": 0, "w": 400, "h": 50}),
                _make_node("e3", "listitem", "Hidden",
                           bounds={"x": 0, "y": 200, "w": 400, "h": 50}),
            ]),
        ])]
        env = _make_envelope(tree)
...
class TestScreenViewportClipping:
    def test_deeply_offscreen_clipped_by_screen_viewport(self):
        ...
        tree = [_make_node("e0", "window", "App",
                           bounds={"x": 0, "y": 0, "w": 800, "h": 600}, children=[
            _make_node("e1", "main", "Content",
                       bounds={"x": 0, "y": 0, "w": 800, "h": 600},
                       actions=["click"], children=[
                # Visible
                _make_node("e2", "button", "Visible Btn",
                           bounds={"x": 10, "y": 10, "w": 100, "h": 30},
                           actions=["click"]),
                # Deeply offscreen — y=3000, screen is 1080 tall
                _make_node("e3", "button", "Far Away",
                           bounds={"x": 10, "y": 3000, "w": 100, "h": 30},
                           actions=["click"]),
                _make_node("e4", "group", "Also Far",
                           bounds={"x": 10, "y": 5000, "w": 400, "h": 500},
                           actions=["click"], children=[
                    _make_node("e5", "button", "Nested Far",
    ...
    def test_screen_viewport_keeps_onscreen_elements(self):
        ...
        tree = [_make_node("e0", "window", "App",
            ...
            _make_node("e1", "main", "Content",
                ...
                _make_node("e2", "button", "Below Parent",
    ...
    def test_no_screen_info_skips_screen_clipping(self):
        ...
        tree = [_make_node("e0", "window", "App", children=[
            _make_node("e1", "button", "Far Button",
    ...
    def test_screen_clipping_shows_hint(self):
        ...
        tree = [_make_node("e0", "window", "App",
                           bounds={"x": 0, "y": 0, "w": 800, "h": 600}, children=[
            # Visible
            _make_node("e1", "button", "Visible",
                       bounds={"x": 10, "y": 10, "w": 100, "h": 30},
                       actions=["click"]),
            # Below screen
            _make_node("e2", "button", "Hidden",
